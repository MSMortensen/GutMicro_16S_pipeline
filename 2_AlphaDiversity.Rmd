...
Formula <- paste("Observed ~", params$batch, sep = " ")
compare_means(as.formula(Formula),  data = adat, method = "kruskal")

## If significant:
# Perform pairwise comparisons
stat.test <- adat %>%
  wilcox_test(as.formula(Formula)) %>%
...
Formula <- paste("Shannon ~", params$batch, sep = " ")
compare_means(as.formula(Formula),  data = adat, method = "kruskal")
...
Formula <- paste("FaithPD ~", params$batch, sep = " ")
compare_means(as.formula(Formula),  data = adat, method = "kruskal")
...
### Observed richness
Formula <- paste("Observed ~", TEST.VAR, sep = " ")
compare_means(as.formula(Formula),  data = adat, method = "kruskal")
...
### Test shannon diversity index
# Run statistical test of batch effect
Formula <- paste("Shannon ~", TEST.VAR, sep = " ")
compare_means(as.formula(Formula),  data = adat, method = "kruskal")
...
# Test Faith phylogenetic distance
Formula <- paste("FaithPD ~", TEST.VAR, sep = " ")
compare_means(as.formula(Formula),  data = adat, method = "kruskal")
...
melted_corrmat_rounded <- melt(corrmat_rounded)

ggplot(melted_corrmat_rounded, aes(x = Var1, y = Var2, fill = value)) + 
